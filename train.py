    ...
    def __call__(self, metrics):
        ...
        for metric in metrics:
            ...
            if localName not in self.histories:
                continue
            self.histories[localName].append(metrics[metric])
        ...
        for metric in self.histories:
            ...
            if self.timing == self.total:
                recent = np.array(self.histories[metric])
...
def trainModel(config, modelClass, dataClass, objective, epochs, criterion: dict[str: nn.Module], resume=None, deltas={}, name=""):
    ...
    stopper = EarlyStop(deltas, threshold=2, timing=4000)
...
# pear2 = Pearson2()
# kge = CMALKGE()
# a = torch.linspace(0, 1, 128).unsqueeze(0).repeat(128, 1).to(device)
# b = torch.linspace(0, 1, 128).unsqueeze(0).repeat(128, 1).to(device)
...
# start = datetime.now()
# for i in range(2000):
#     results.append(pear1(a, b))
# total1 = datetime.now() - start

# start = datetime.now()
# for i in range(2000):
#     p2 = pear2(a, b)
#     if torch.amax(results[i].squeeze() - p2.squeeze()) > 1e-3:
#         print("Wrong", results[i].shape, p2.shape, torch.amax(results[i].squeeze() - p2.squeeze()))
# total2 = datetime.now() - start

# print(f"P1 time: {total1.total_seconds()} | P2 time: {total2.total_seconds()}")

# print(kge(a, b, torch.tensor([0.5]).to(device), torch.tensor([0.25]).to(device)))
...
# stopper = EarlyStop(deltas, threshold=2)

# for i in range(20000):
#     stopper({"Test NSE": -i, "Test NMAE": i})
...
models = [ComboBlockStation, FloodHub]
datasets = [InundationData, FloodHubData]
configs = ["Combo GCNBlock5Config.json", "FloodHubConfig.json"]
