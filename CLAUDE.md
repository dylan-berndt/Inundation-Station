...
- `trainModel(..., useAMP=True, accumulationSteps=1)`: `useAMP` runs the forward pass and loss in bf16 autocast (on by default, no-op on CPU; no `GradScaler`, since bf16 doesn't need one) — memory, not compute, is what `nodesPerBatch` is bottlenecked on. `accumulationSteps>1` delays `optimizer.step()`/`zero_grad()` across that many batches, trading wall-clock for a larger effective batch without the memory cost, once AMP alone isn't enough headroom.
