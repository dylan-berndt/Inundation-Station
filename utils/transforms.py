...
class Transform:
    ...
    def __init__(self, forward, backward, mode="linear", stats=None):
    ...
    def __repr__(self):
        stats = ", ".join(f"{key}={value:.6g}" for key, value in self.stats.items())
        return f"Transform(mode={self.mode!r}, {stats})"
...
# config.targetTransform. Each entry is (warp, inverse warp); both accept
# numpy arrays and torch tensors, and both are pure functions of the value
# with no per-gauge state, so they stay invertible at an ungauged basin.
WARPS = {
    ...
    "cbrt": (lambda x: _signedPower(x, 1.0 / 3.0), lambda x: _signedPower(x, 3.0)),
    "sqrt": (lambda x: _signedPower(x, 0.5), lambda x: _signedPower(x, 2.0)),
    # Kept for comparison with the pre-2026-01 runs, which used it.
    "log": (
        lambda x: (torch.log10(torch.clamp(x, min=1e-6)) if isinstance(x, torch.Tensor)
                   else np.log10(np.clip(x, 1e-6, None))),
        lambda x: (torch.pow(10.0, x) if isinstance(x, torch.Tensor) else np.power(10.0, x)),
    ),
}
...
def streamflowProcess(targets, mode="cbrt"):
    ...
    return Transform(
        lambda x: (warp(x) - mean) / std,
        lambda x: unwarp((std * x) + mean),
        mode=mode,
...
if __name__ == "__main__":
    # Round-trip check on a heavy-tailed sample resembling specific discharge.
    rng = np.random.default_rng(0)
    q = np.clip(rng.lognormal(-5, 1.4, 200000), 0, None)
    for mode in WARPS:
        t = streamflowProcess(q, mode=mode)
        x = torch.tensor(q[:2048], dtype=torch.float32)
        back = t.backward(t.forward(x))
        print(f"{mode:7s} max |round-trip error| = {(back - x).abs().max().item():.3e}   {t}")
